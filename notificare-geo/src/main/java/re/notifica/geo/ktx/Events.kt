...
@Suppress("unused")
internal suspend fun NotificareEventsModule.logRegionSession(session: NotificareRegionSession): Unit =
...
@Suppress("unused")
internal suspend fun NotificareEventsModule.logBeaconSession(session: NotificareBeaconSession): Unit =
    withContext(Dispatchers.IO) {
        val sessionEnd = session.end ?: Date()
        val sessionLength = (sessionEnd.time - session.start.time) / 1000.0
